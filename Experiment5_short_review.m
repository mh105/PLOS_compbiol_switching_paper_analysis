%% Switching state-space models for sleep spindle detection and analysis >>> NEW UPDATES FOR REVIEWERS
% Version: 03/03/2023 Alex He - switched to using the new ssm class 
close all
clear all
clc
...
[Mprob, fy_t, obj_array, A, VB_iter, Mprob_soft, Mprob_hard, logL_bound, x_t_n_all, P_t_n_all] = VBlearn(o1, 'shared_ctype', {[1,1]}, 'norm_qt_m', true);
...
axes(ax(5))
extracted_spindle = x_t_n_all{1}(3,2:end);
extracted_spindle(Mprob(1,:) < 0.5) = 0;
spindle_2std = squeeze(sqrt(P_t_n_all{1}(3,3,2:end))*2);
spindle_2std(Mprob(1,:) < 0.5) = 0;

shadedErrorBar(t1, extracted_spindle, spindle_2std, {'Color', [0 0.4470 0.7410]});

% get the rotation frequency as well
...
axes(ax(6))
extracted_spindle = x_t_n_all{1}(4,2:end);
extracted_spindle(Mprob(1,:) < 0.5) = 0;
spindle_2std = squeeze(sqrt(P_t_n_all{1}(4,4,2:end))*2);
spindle_2std(Mprob(1,:) < 0.5) = 0;
...
axes(ax(7))
plot(t1, Mprob_soft(1,:), 'LineWidth',2);
hold on
plot(t1, Mprob_hard(1,:), 'LineWidth',2);
title('Soft segmentation and Viterbi')
set(gca, 'FontSize', 14)
...
figure
ax = figdesign(12,1, 'merge', {[1,2]}, 'margin', [.05 .1 .1 .05 .02]);
set(gcf, 'Position', [1 0.0810 0.3000 0.7382])
for ii = 1:length(ax);  axes(ax(ii)); title(num2str(ii)); end
linkaxes(ax, 'x')
