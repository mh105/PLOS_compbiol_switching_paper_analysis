...
function [x_t_n,P_t_n, P_t_tmin1_n, logL, x_t_t,P_t_t, K_t, x_t_tmin1,P_t_tmin1] = djkalman_conv(F, Q, mu0, Q0, G, R, y, conv_steps)
...
for ii=2:conv_steps+1 % t=1 -> t=conv_steps
    ...
    logL(ii-1) = -1/2 * (qlog2pi + sparse_find_log_det_mex(D_t) + e_t(:,ii)'*invD_t*e_t(:,ii)); 
end
