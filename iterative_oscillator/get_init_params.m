function [Phi_init, Q_init, R_init, mu_init, sigma_init ,NNharm, f_tot_curWind, w_init] = get_init_params(init_params,Fs,windId,Nosc_max)
...
mu_init     = zeros(2*Nosc,1);         % Initial state means (t=0)
sigma_init  = 3*eye(2*Nosc);           % Initial State covariances (t=0) --- note that this is different from sigma2_init which casts into Q_init

Phi_init    = zeros(2*Nosc);           % State Transition matrix
Q_init      = zeros(2*Nosc);           % State/process noise

w_init = zeros(1,Nosc_ind); %initial frequency in radians

for n_osc_ind= 1:Nosc_ind
    w_n = (1:NNharm (1,n_osc_ind)) * f_tot_curWind(1,n_osc_ind) * 2 * pi / Fs;
    w_init(n_osc_ind) = w_n(1); %altered from osc_harm
    ...
    for nn_osc= 1 : NNharm (1,n_osc_ind)
        ...
        Q_init  (curId:curId+1, curId:curId+1)= eye(2)*sigma2_n; % --- note that sigma2_init gets output as Q_init for state noise 
    end
end
